...
class LoadoutScreen(
    ...
    private fun drawLoadouts(graphics: GuiGraphicsExtractor, mouseX: Int, mouseY: Int) {
        ...
        visible.forEachIndexed { position, loadout ->
            ...
            drawLoadout(graphics, loadout, x, y, mouseX, mouseY)
    ...
    private fun drawLoadout(
        ...
        if (loadout.selected) {
            drawOutline(graphics, x - 4, gridY - 4, pageWidth + 8, slotSize + 8, SkyHudTheme.PRIMARY_HOVER)
        }
        ...
        val displayItems = if (loadout.items.any { !it.isEmpty }) {
            loadout.items
        } else {
            List(9) { index -> if (index == 4) loadout.selector else ItemStack.EMPTY }
        }
        val itemInset = (slotSize - itemSize) / 2
        repeat(9) { index ->
            val stack = displayItems.getOrNull(index) ?: ItemStack.EMPTY
            val slotX = x + index * (slotSize + slotGap)
    ...
    override fun mouseClicked(click: MouseButtonEvent, doubled: Boolean): Boolean {
        ...
        if (loadout.clickable) clickBackingSlot(loadout.inventorySlot)
        return true
    }
