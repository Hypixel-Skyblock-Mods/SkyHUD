...
object ToolkitIcons {
    ...
    val farming: ItemStack by lazy {
        runCatching { playerHead("SkyHUD Farming Toolkit", farmingTexture) }
            .getOrElse { ItemStack(Items.PLAYER_HEAD) }
    }
    val hunting: ItemStack by lazy {
        ...
        }.getOrElse { ItemStack(Items.PAPER) }
    }
    ...
    private fun playerHead(key: String, texture: String): ItemStack {
        ...
        val profile = GameProfile(UUID.nameUUIDFromBytes(key.toByteArray(StandardCharsets.UTF_8)), key, properties)
